...
class LazyFrames:
    """只在真正轉成 numpy array 時才把多張影格堆疊起來。"""
    __slots__ = ("frames", "out")
    def __init__(self, frames):
        self.frames = frames      # list[np.ndarray, uint8]
        self.out = None
    def __array__(self, dtype=None):
        if self.out is None:
            self.out = np.stack(self.frames, axis=0)      # (4,84,84)
...
class AtariPreprocessor:
    ...
    def preprocess(self, obs):
        ...
        ## 保存統一改成uint8
        return resized.astype(np.uint8)          # 不做 /255，此時還是 0–255
    def reset(self, obs):
...
class PrioritizedReplayBuffer:
    ...
    def __init__(self, capacity, alpha=0.6, beta=0.4):
        ...
        self.obs_store = np.empty((capacity, 84, 84), dtype=np.uint8)
