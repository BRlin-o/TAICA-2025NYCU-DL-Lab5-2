...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        # Convert states and next_states to tensors (as done in original code)
        state_tensors = [torch.from_numpy(np.array(s)).float() for s in states]
        next_state_tensors = [torch.from_numpy(np.array(s)).float() for s in next_states]
        actions_np = np.array(actions)
        rewards_np = np.array(rewards, dtype=np.float32)
        dones_np = np.array(dones, dtype=np.float32)

        # Determine if CUDA is available for pinning
        use_cuda = torch.cuda.is_available()

        # Stack state tensors, pin only if CUDA
...
class DQNAgent:
    def __init__(self, env_name="ALE/Pong-v5", args=None):
        ...
        # Always treat as Atari (normalize by 255)
        ...
        self.q_net = DQN(self.num_actions, self.input_shape, use_cnn=True).to(self.device)
        self.q_net.apply(init_weights)
        self.target_net = DQN(self.num_actions, self.input_shape, use_cnn=True).to(self.device)
        ...
        while self.episode < episodes:
            ...
            while not done and step_count < self.max_episode_steps:
                ...
                if len(self.nstep_queue) == self.n_step:
                    ...
                    # 計算TD誤差
                    with torch.no_grad():
                        state_tensor = torch.from_numpy(np.array(state_0)).float().unsqueeze(0).to(self.device) / 255.0
    ...
    def train(self):
        ...
        # Use non_blocking transfers only for CUDA devices
        cuda_non_blocking = (self.device.type == "cuda")

        # Move to device asynchronously; normalize if Atari
        # (This code always uses Atari preprocessing, so always normalize by 255.0)
        states = states.to(self.device, non_blocking=cuda_non_blocking) / 255.0
        next_states = next_states.to(self.device, non_blocking=cuda_non_blocking) / 255.0
        actions = actions.to(self.device, non_blocking=cuda_non_blocking)
        rewards = rewards.to(self.device, non_blocking=cuda_non_blocking)
        dones = dones.to(self.device, non_blocking=cuda_non_blocking)
        weights = weights.to(self.device, non_blocking=cuda_non_blocking)
