...
class DQNAgent:
    def __init__(self, env_name="ALE/Pong-v5", args=None):
        # Create Atari preprocessing + frame stacking wrappers
        env = gym.make(env_name, render_mode="rgb_array")
        env = AtariPreprocessing(env, screen_size=84, grayscale_obs=True, frame_skip=args.frame_skip)
        self.env = FrameStack(env, num_stack=4)

        # Separate test environment
        test_env = gym.make(env_name, render_mode="rgb_array")
        test_env = AtariPreprocessing(test_env, screen_size=84, grayscale_obs=True, frame_skip=args.frame_skip)
        self.test_env = FrameStack(test_env, num_stack=4)

        self.num_actions = self.env.action_space.n
        # Input shape inferred from stacked wrapper
        self.input_shape = self.env.observation_space.shape  # e.g., (4, 84, 84)
        ...
        self.env_count = 0
        self.train_count = 0
        self.best_reward = -21  # Initilized to -21 for Pong (Atari)
    ...
    def select_action(self, state):
        ...
        # state comes as a numpy array of shape (4,84,84), values 0–255
    ...
    def train(self):
        ...
        # Convert to torch tensors and move to device with pinned memory for faster transfer
        states = torch.from_numpy(np.array(states)).float().pin_memory().to(self.device, non_blocking=True) / 255.0
        next_states = torch.from_numpy(np.array(next_states)).float().pin_memory().to(self.device, non_blocking=True) / 255.0
