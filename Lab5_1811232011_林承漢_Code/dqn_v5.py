...
class LazyFrames:
    """
    延遲把多張影格疊成 (stack, 84, 84)；只有在真的
    被轉成 numpy / torch tensor 時才做 np.stack，
    讓不同 transition 共享同一張影格、避免重複佔 RAM
    """
    __slots__ = ("frames", "_out")

    def __init__(self, frames):
        self.frames = frames            # list[np.ndarray] each shape (84,84) uint8
        self._out = None                # cache for np.stack(...)

    def __array__(self, dtype=None):
        if self._out is None:
            self._out = np.stack(self.frames, axis=0)   # (stack,84,84)
        return self._out.astype(dtype) if dtype else self._out
...
class AtariPreprocessor:
    ...
    """    
    def __init__(self, frame_stack=4, use_uint8=True):
    ...
    def preprocess(self, obs):
        ...
        return resized.astype(np.uint8) if self.use_uint8 else resized / 255.0
    def reset(self, obs):
...
class DQNAgent:
    def __init__(self, env_name="ALE/Pong-v5", args=None):
        ...
        if self.is_atari:
            # Atari 環境使用圖像輸入
            self.preprocessor = AtariPreprocessor(use_uint8=self.use_uint8)
            self.input_shape = (4, 84, 84)  # 4 個堆疊的灰度圖像
        else:
            # CartPole 環境使用向量輸入
            self.preprocessor = AtariPreprocessor(use_uint8=self.use_uint8)  # 保持一致性，但實際上不會用於預處理
        ...
        # self.memory = deque(maxlen=args.memory_size) # Task1, 2
    ...
    def select_action(self, state):
        ...
        if self.is_atari and self.use_uint8:
            state_tensor = torch.from_numpy(state_arr).float().div_(255.0).unsqueeze(0).to(self.device)
        else:
    ...
    def train(self):
        ...
        (states, actions, rewards, next_states,
         dones, indices, weights) = self.memory.sample(self.batch_size)
        ########## END OF YOUR CODE ##########
        ...
        self.memory.beta = min(1.0,
                                self.memory.beta + (1.0 - self.memory.beta_start) / self.linear_decay_steps)
    ...
    def save_checkpoint(self, name="latest.pt", is_best=False, is_periodic=False):
        ...
        ckpt = {
            ...
            # "memory": list(self.memory),  # removed to avoid duplicating whole buffer
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-step", type=int, default=3,
                        help="n‑step return length")
    parser.add_argument("--per-alpha", type=float, default=0.6,
                        help="priority exponent α")
    parser.add_argument("--per-beta-start", type=float, default=0.4,
                        help="initial importance‑sampling exponent β")
    parser.add_argument("--use-uint8", action="store_true",
                        help="store stacked frames as uint8 (0-255) to save RAM; default stores float32 in [0,1]")
