...
class LazyFrames:
    ...
    def __array__(self, dtype=None):
        if self.out is None:
            ...
            self.out = np.stack(np_frames, axis=0)
...
def huber_loss(x, delta=1.0):
    mask = torch.abs(x) < delta
    return torch.where(mask, 0.5 * x**2, delta * (torch.abs(x) - 0.5 * delta))

def init_special_weights(m):
    """特殊初始化 - 針對Pong環境優化"""
    if isinstance(m, nn.Conv2d):
        # 使用特殊分布初始化卷積層
        nn.init.kaiming_normal_(m.weight, nonlinearity='relu', mode='fan_out')
        if m.bias is not None:
            nn.init.constant_(m.bias, 0.1)  # 小正偏置
...
class DQN(nn.Module):
    def __init__(self, num_actions, input_shape, use_cnn=False):
        ...
        if not use_cnn:
            ...
            self.value_stream = nn.Linear(64*7*7, 1)
            self.advantage_stream = nn.Linear(64*7*7, num_actions)
...
class AtariPreprocessor:
    ...
    """    
    def __init__(self, frame_stack=4, device=torch.device("cpu")):
    ...
    def reset(self, obs):
        ...
        # return LazyFrames(list(self.frames), device=self.device)
...
class DQNAgent:
    def __init__(self, env_name="ALE/Pong-v5", args=None):
        ...
        if self.is_atari:
            # Atari 環境使用圖像輸入
            self.preprocessor = AtariPreprocessor(device=self.device)
            self.input_shape = (4, 84, 84)  # 4 個堆疊的灰度圖像
        else:
            # CartPole 環境使用向量輸入
            self.preprocessor = AtariPreprocessor(device=self.device)  # 保持一致性，但實際上不會用於預處理
        ...
        self.q_net.apply(init_special_weights)
    ...
    def evaluate(self):
        ...
        while not done:
            ...
            if hasattr(state, 'frames'):  # 檢查是否為LazyFrames對象
                # 直接處理frames列表
                frames = [np.array(f) for f in state.frames]
                state_array = np.stack(frames, axis=0)
                state_tensor = torch.from_numpy(state_array).float().unsqueeze(0)
    ...
    def train(self):
        ...
        abs_errors = torch.abs(td_errors).detach().cpu().numpy()
        self.memory.update_priorities(indices, abs_errors ** 1.2)
        ...
        # else:
        #     # 每次訓練步驟都進行軟更新
        #     self.soft_update(tau=0.001)  # 使用較小的tau確保穩定性
